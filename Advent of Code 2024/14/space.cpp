...
size_t Space::calculateSafetyFactorAfter(size_t seconds)
{
	...
	for ( int i = 0, height = fSize.fY / 2; i < height; i++ )
		for ( int j = fSize.fX / 2 + 1; j < fSize.fX; j++ )
			quad1Num	+= fRobotGrid[ i ][ j ];

	for ( int i = 0, height = fSize.fY / 2; i < height; i++ )
		for ( int j = 0, width = fSize.fX / 2; j < width; j++ )
			quad2Num	+= fRobotGrid[ i ][ j ];

	for ( int i = fSize.fY / 2 + 1; i < fSize.fY; i++ )
		for ( int j = 0, width = fSize.fX / 2; j < width; j++ )
...
{
	const int			iterations		{ fSize.fX * fSize.fY };
